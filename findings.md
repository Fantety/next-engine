...
## 2026-06-01 NEXT Editable Plan Findings
- Plan editing should remain a workflow-session operation, not a 2.3 plan-management unification. The session layer now mediates user edits so workflow-active and locked-milestone guards stay centralized.
- The state layer needed first-class mutation APIs for user-owned plan edits: milestone delete/move/merge, task delete/move, and task dependency replacement. Existing update/add paths also needed locked-milestone rejection so UI and non-UI callers share the same protection.
- Task dependency edits require validation before durable persistence: unknown tasks, self-dependencies, and dependency cycles are rejected, with rollback for dependency mutations.
- Existing task reordering had a hidden same-milestone bug: moving a task down by one passed `to_index = i + 1`, but `move_task()` also adjusted the index after removal, cancelling the move. The API now treats `to_index` as the final target index, matching milestone reordering.
- Compact UI rows need stable test/discovery metadata because Godot enforces unique sibling node names. Repeated rows are added with readable unique names and tests match the `TaskRow*` / `MilestoneRow*` prefixes.
- User requested direct custom icon names only, so new plan-editing controls use the listed `AIPlan*` icon names directly rather than local fallback names.
- The milestone/task row UI is now a good component boundary: rows own compact layout, icon buttons, metadata, and drag forwarding setup; parent list/tree code owns dialogs, session mutation calls, drop validation/execution, and refresh lifecycle. This keeps behavior centralized while making future row polish local to `ai_next_plan_rows.*`.
