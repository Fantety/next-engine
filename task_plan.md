...
- [complete] Phase 12: Add editable NEXT plans and compact plan UI: user milestone/task edits, dependency editing, drag reorder, locked/running edit guards, and direct custom AIPlan icon names.
- [complete] Phase 13: Extract NEXT milestone/task row UI into focused reusable components without changing plan-editing behavior.
...
| Repeated `TaskRow` UI test counted only one row after rendering three tasks. | NEXT editable plan UI verification | Root cause was Godot sibling node-name uniqueness; rows are now added with readable unique names and tests match `TaskRow*`. |
