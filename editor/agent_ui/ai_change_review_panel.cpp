...
AIChangeReviewPanel::AIChangeReviewPanel() {
	...
	if (bridge.is_valid()) {
		const Callable change_sets_changed = callable_mp(this, &AIChangeReviewPanel::_change_sets_changed);
		if (!bridge->is_connected(SNAME("change_sets_changed"), change_sets_changed)) {
			bridge->connect(SNAME("change_sets_changed"), change_sets_changed, CONNECT_DEFERRED);
		}
	}
...
void AIChangeReviewPanel::_change_sets_changed() {
	_refresh();
}
...
void AIChangeReviewPanel::_refresh() {
	...
	Array changes = bridge->list_change_sets("pending");
...
void AIChangeReviewPanel::_view_change_set(const String &p_change_set_id) {
	if (bridge.is_null() || !diff_dialog || !diff_viewer) {
		return;
	}

	Dictionary change_set = bridge->get_change_set(p_change_set_id);
...
void AIChangeReviewPanel::_keep_change_set(const String &p_change_set_id) {
	...
	const Dictionary result = bridge->keep_change_set(p_change_set_id);
	if (!bool(result.get("success", false))) {
		const Dictionary error = result.get("error", Dictionary());
		_show_error(String(error.get("message", TTR("Could not keep AI change."))));
	}
	_refresh();
}
...
void AIChangeReviewPanel::_confirm_revert_change_set() {
	if (bridge.is_null() || pending_revert_change_set_id.is_empty()) {
		pending_revert_change_set_id.clear();
		return;
	}
	const Dictionary result = bridge->revert_change_set(pending_revert_change_set_id);
	if (!bool(result.get("success", false))) {
		const Dictionary error = result.get("error", Dictionary());
		_show_error(String(error.get("message", TTR("Could not revert AI change."))));
	}
	pending_revert_change_set_id.clear();
	_refresh();
}
