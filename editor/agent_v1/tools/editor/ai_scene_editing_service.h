...
class AIV1SceneEditingService : public AIV1EditorToolService {
	GDCLASS(AIV1SceneEditingService, AIV1EditorToolService);
	...
	static AIV1SceneEditingService *get_dispatcher_singleton();
	void _execute_request(uint64_t p_request_ptr);
	void _execute_request_ptr(MainThreadRequest *p_request);
	AIV1SceneEditingResult _dispatch_to_main_thread(MainThreadRequest &r_request);
	...
	AIV1SceneEditingResult _delete_node_main_thread(const String &p_node_path);
	AIV1SceneEditingResult _list_properties_main_thread(const String &p_node_path, const String &p_filter, int p_max_properties, bool p_include_read_only, bool p_include_current_values);
	AIV1SceneEditingResult _inspect_node_main_thread(const String &p_node_path, const Array &p_property_paths);
	AIV1SceneEditingResult _describe_tree_main_thread(const String &p_root_path, int p_max_depth, int p_max_nodes, bool p_include_internal);
	AIV1SceneEditingResult _apply_patch_main_thread(const Dictionary &p_patch);
...
public:
	AIV1SceneEditingResult delete_node(const String &p_node_path);
	AIV1SceneEditingResult list_properties(const String &p_node_path, const String &p_filter, int p_max_properties, bool p_include_read_only, bool p_include_current_values);
	AIV1SceneEditingResult inspect_node(const String &p_node_path, const Array &p_property_paths);
	AIV1SceneEditingResult describe_tree(const String &p_root_path, int p_max_depth, int p_max_nodes, bool p_include_internal);
	AIV1SceneEditingResult apply_patch(const Dictionary &p_patch);
};
