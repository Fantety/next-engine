...
void AIChangeSetStore::clear_singleton_for_test() {
	if (singleton.is_valid()) {
		singleton->clear_for_test();
	}
	singleton.unref();
}
		...
		for (int change_index = 0; change_index < p_changes.size(); change_index++) {
			...
			if (!path.is_empty()) {
				...
				if (existing_index >= 0) {
					...
					Dictionary change_set_metadata = updated_change_set.get("metadata", Dictionary()).duplicate(true);
					for (const KeyValue<Variant, Variant> &E : p_metadata) {
						change_set_metadata[E.key] = E.value;
					}
					change_set_metadata["merged_review_change"] = true;
					change_set_metadata["last_title"] = p_title;
					change_set_metadata["last_tool_call_id"] = p_tool_call_id;
					updated_change_set["metadata"] = change_set_metadata;
