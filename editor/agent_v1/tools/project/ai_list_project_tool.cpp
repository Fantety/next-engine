...
String AIV1ListProjectTool::get_description() const {
	return "Lists files and directories under res:// with depth and entry limits.";
}

Dictionary AIV1ListProjectTool::get_parameters_schema() const {
	Dictionary schema;
	schema["type"] = "object";
...
AIV1EditorToolResult AIV1ListProjectTool::execute_tool(const Dictionary &p_arguments) {
	...
	max_depth = AIV1ProjectToolUtils::get_int_argument(p_arguments, "max_depth", 4, 0, 8);
	max_entries = AIV1ProjectToolUtils::get_int_argument(p_arguments, "max_entries", 400, 1, 2000);
	max_chars = AIV1ProjectToolUtils::get_int_argument(p_arguments, "max_chars", 16000, 1000, 64000);
	entry_count = 0;
	truncated = false;
...
void AIV1ListProjectTool::_append_tree(String &r_output, const String &p_path, int p_depth) {
