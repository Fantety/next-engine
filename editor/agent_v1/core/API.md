...
本文档描述 `editor/agent_v1/core` 中已经建立的基础设施和运行时契约。它的目标是给后续复刻 opencode 架构时提供稳定参考：Provider、Runner、ToolRegistry、MCP、Skill、EventStore 等模块应优先依赖这里的中性类型，而不是依赖 `editor/agent_ui` 的界面状态或某个具体 provider 的数据格式。
...
- 是否能在不中断 `editor/agent_ui` 展示层的前提下演进？
