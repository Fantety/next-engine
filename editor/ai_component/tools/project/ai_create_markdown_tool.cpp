...
String AICreateMarkdownTool::get_description() const {
	return "Creates or overwrites a Markdown document under res://.";
}

Dictionary AICreateMarkdownTool::get_parameters_schema() const {
	Dictionary schema;
	schema["type"] = "object";
...
AIToolResult AICreateMarkdownTool::execute(const Dictionary &p_arguments) {
	AIToolResult result;
