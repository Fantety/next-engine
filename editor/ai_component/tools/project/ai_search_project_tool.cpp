...
String AISearchProjectTool::get_name() const {
	return "project.search_text";
}

String AISearchProjectTool::get_description() const {
	return "Searches allowlisted text files under res:// using literal substring matching.";
}

Dictionary AISearchProjectTool::get_parameters_schema() const {
	Dictionary schema;
	schema["type"] = "object";
...
void AISearchProjectTool::_search_dir(String &r_output, const String &p_path) {
	...
	while (!entry.is_empty()) {
		if (entry != "." && entry != ".." && !entry.begins_with(".")) {
			...
			} else if (AIProjectToolUtils::is_allowed_text_extension(child_path)) {
...
void AISearchProjectTool::_search_file(String &r_output, const String &p_path) {
