...
AIDocsSearchTool::AIDocsSearchTool() {
	service.instantiate();
}

String AIDocsSearchTool::get_name() const {
	return "docs.search";
}

String AIDocsSearchTool::get_description() const {
...
Dictionary AIDocsSearchTool::get_parameters_schema() const {
	Dictionary schema;
	schema["type"] = "object";
...
AIToolResult AIDocsSearchTool::execute(const Dictionary &p_arguments) {
	AIToolResult result;
	...
	AIDocumentationResult doc_result = service->search(query, class_name, kind, max_results, include_descriptions);
