...
void AISceneEditingService::_execute_request_ptr(MainThreadRequest *p_request) {
	...
	switch (p_request->operation) {
		...
		case MainThreadRequest::OP_DELETE_NODE:
			...
			break;
		case MainThreadRequest::OP_RENAME_NODE:
			p_request->result = _rename_node_main_thread(p_request->node_path, p_request->new_name);
			break;
		case MainThreadRequest::OP_MOVE_NODE:
			p_request->result = _move_node_main_thread(p_request->node_path, p_request->new_parent_path, p_request->position);
			break;
		case MainThreadRequest::OP_SET_PROPERTY:
			p_request->result = _set_property_main_thread(p_request->node_path, p_request->property_path, p_request->value);
			break;
		case MainThreadRequest::OP_LIST_PROPERTIES:
			...
			break;
		case MainThreadRequest::OP_SAVE_CURRENT_SCENE:
			p_request->result = _save_current_scene_request_main_thread();
			break;
		case MainThreadRequest::OP_OPEN_SCENE:
			p_request->result = _open_scene_main_thread(p_request->scene_path);
			break;
	}
...
AISceneEditingResult AISceneEditingService::_rename_node_main_thread(const String &p_node_path, const String &p_new_name) {
	AISceneEditingResult result;
	String error;
	Node *scene = _get_edited_scene(error);
	if (!scene) {
	...
	if (node->is_internal()) {
		result.error = "Internal nodes cannot be renamed by the AI scene tool.";
		return result;
	}

	const String old_name = node->get_name();
	const String requested_name = p_new_name.strip_edges();
	if (requested_name.is_empty()) {
		result.error = "New node name is required.";
		return result;
	}
	...
	if (new_name.is_empty()) {
		result.error = "New node name is invalid.";
		return result;
	}
	new_name = parent->prevalidate_child_name(node, new_name);
	if (new_name == old_name) {
		result.success = true;
		result.message = vformat("Node `%s` already has the requested name.", p_node_path);
		result.metadata["node_path"] = String(scene->get_path_to(node));
		result.metadata["node_name"] = new_name;
		result.metadata["scene_path"] = current_scene_path;
		result.metadata["saved"] = true;
		return result;
	}
	...
	const NodePath old_path = scene->get_path_to(node);
	undo_redo->create_action_for_history(TTR("AI Rename Node"), EditorNode::get_editor_data().get_current_edited_scene_history_id());
	...
	if (dock) {
		...
		Vector<StringName> new_path_names = scene->get_path_to(parent).get_names();
		new_path_names.push_back(new_name);
		path_renames[node] = NodePath(new_path_names, true);
		dock->perform_node_renames(nullptr, &path_renames);
	}

	EditorDebuggerNode *debugger = EditorDebuggerNode::get_singleton();
	if (debugger) {
		undo_redo->add_do_method(debugger, "live_debug_reparent_node", old_path, scene->get_path_to(parent), new_name, node->get_index(false));
		undo_redo->add_undo_method(debugger, "live_debug_reparent_node", NodePath(String(scene->get_path_to(parent)).path_join(new_name)), scene->get_path_to(parent), old_name, node->get_index(false));
	}
	undo_redo->commit_action();
	...
	if (!_save_current_scene_main_thread(scene, saved_path, error)) {
		...
		result.metadata["saved"] = false;
		result.metadata["node_path"] = p_node_path;
		result.metadata["node_name"] = old_name;
		result.metadata["scene_path"] = current_scene_path;
		return result;
	}

	result.success = true;
	result.message = vformat("Renamed node `%s` to `%s` and saved `%s`.", p_node_path, new_name, saved_path);
	result.metadata["node_path"] = String(scene->get_path_to(node));
	result.metadata["node_name"] = new_name;
	result.metadata["old_name"] = old_name;
	result.metadata["scene_path"] = saved_path;
	result.metadata["saved"] = true;
	return result;
}

AISceneEditingResult AISceneEditingService::_move_node_main_thread(const String &p_node_path, const String &p_new_parent_path, int p_position) {
	AISceneEditingResult result;
	String error;
	Node *scene = _get_edited_scene(error);
	if (!scene) {
	...
	Node *new_parent = _resolve_node_path(scene, p_new_parent_path, true, error);
	if (!new_parent) {
		result.error = error;
		return result;
	}
	if (node == new_parent) {
		result.error = "A node cannot be moved under itself.";
		return result;
	}
	if (node->is_ancestor_of(new_parent)) {
		result.error = "A node cannot be moved under one of its descendants.";
		return result;
	}
	if (node->is_internal()) {
		result.error = "Internal nodes cannot be moved by the AI scene tool.";
		return result;
	}
	...
	int target_position = p_position;
	if (target_position < 0) {
		target_position = same_parent ? old_index : new_parent->get_child_count(false);
	}
	if (same_parent && target_position == old_index) {
		result.success = true;
		result.message = vformat("Node `%s` is already at the requested position.", p_node_path);
		result.metadata["node_path"] = String(scene->get_path_to(node));
		result.metadata["parent_path"] = String(scene->get_path_to(new_parent));
		result.metadata["position"] = target_position;
		result.metadata["scene_path"] = current_scene_path;
		result.metadata["saved"] = true;
		return result;
	}
	...
	const NodePath old_path = scene->get_path_to(node);
	const NodePath new_parent_path = scene->get_path_to(new_parent);
	const int new_position = CLAMP(target_position, 0, new_parent->get_child_count(false));
	SceneTreeDock *dock = SceneTreeDock::get_singleton();
	HashMap<Node *, NodePath> path_renames;
	if (dock) {
		dock->fill_path_renames(node, new_parent, &path_renames);
	}

	undo_redo->create_action_for_history(TTR("AI Move Node"), EditorNode::get_editor_data().get_current_edited_scene_history_id(), UndoRedo::MERGE_DISABLE, false, true);
	if (!same_parent) {
	...
	undo_redo->add_do_method(node, "set_name", new_name);
	if (dock) {
		undo_redo->add_do_method(dock, "_set_owners", scene, owners);
	}
	...
	EditorDebuggerNode *debugger = EditorDebuggerNode::get_singleton();
	if (debugger) {
		undo_redo->add_do_method(debugger, "live_debug_reparent_node", old_path, new_parent_path, new_name, new_position);
		undo_redo->add_undo_method(debugger, "live_debug_reparent_node", NodePath(String(new_parent_path).path_join(new_name)), scene->get_path_to(old_parent), old_name, old_index);
	}
	...
	if (!_save_current_scene_main_thread(scene, saved_path, error)) {
		...
		result.metadata["saved"] = false;
		result.metadata["node_path"] = p_node_path;
		result.metadata["parent_path"] = p_new_parent_path;
		result.metadata["position"] = p_position;
		result.metadata["scene_path"] = current_scene_path;
		return result;
	}

	result.success = true;
	result.message = vformat("Moved node `%s` under `%s` and saved `%s`.", p_node_path, p_new_parent_path, saved_path);
	result.metadata["node_path"] = String(scene->get_path_to(node));
	result.metadata["parent_path"] = String(scene->get_path_to(new_parent));
	result.metadata["position"] = new_position;
	result.metadata["scene_path"] = saved_path;
	result.metadata["saved"] = true;
	return result;
}

AISceneEditingResult AISceneEditingService::_set_property_main_thread(const String &p_node_path, const String &p_property_path, const Variant &p_value) {
	AISceneEditingResult result;
	String error;
	Node *scene = _get_edited_scene(error);
	if (!scene) {
	...
	if (stripped_property_path.is_empty()) {
		result.error = "Property path is required.";
		return result;
	}
	if (stripped_property_path.begins_with("/") || stripped_property_path.contains("..")) {
		result.error = "Only property paths relative to the target node are allowed.";
		return result;
	}
	...
	} else if (_normalize_property_path(stripped_property_path, property_names, error)) {
	} else {
		result.error = error;
		return result;
	}
	property_path = NodePath(Vector<StringName>(), property_names, false);

	bool valid = false;
	Variant current_value = node->get_indexed(property_names, &valid);
	if (!valid) {
		result.error = vformat("Property `%s` was not found on node `%s`.", p_property_path, p_node_path);
		return result;
	}
	if (listed_property && (listed_property_info.usage & PROPERTY_USAGE_READ_ONLY)) {
		result.error = vformat("Property `%s` is read-only.", p_property_path);
		return result;
	}

	PropertyInfo indexed_property_info;
	const bool indexed_property_found = listed_property ? true : _find_indexed_property_info(node, property_names, indexed_property_info);
	if (listed_property) {
	...
	Variant converted_value;
	Variant::Type target_type = indexed_property_found ? indexed_property_info.type : current_value.get_type();
	if (target_type == Variant::NIL) {
		target_type = current_value.get_type();
	}
	if (!_convert_value_for_property(p_value, indexed_property_found ? &indexed_property_info : nullptr, target_type, converted_value, error)) {
		result.error = error;
		return result;
	}
	...
	if (!_save_current_scene_main_thread(scene, saved_path, error)) {
		...
		result.metadata["saved"] = false;
		result.metadata["node_path"] = p_node_path;
		result.metadata["property_path"] = p_property_path;
		result.metadata["scene_path"] = current_scene_path;
		return result;
	}

	result.success = true;
	result.message = vformat("Set property `%s` on `%s` and saved `%s`.", p_property_path, p_node_path, saved_path);
	result.metadata["node_path"] = p_node_path;
	result.metadata["property_path"] = p_property_path;
	result.metadata["property_type"] = Variant::get_type_name(target_type);
	if (converted_value.get_type() == Variant::OBJECT) {
		Ref<Resource> resource_value = converted_value;
		if (resource_value.is_valid()) {
			result.metadata["resource_type"] = resource_value->get_class();
		}
	}
	result.metadata["scene_path"] = saved_path;
	result.metadata["saved"] = true;
...
AISceneEditingResult AISceneEditingService::_list_properties_main_thread(const String &p_node_path, const String &p_filter, int p_max_properties, bool p_include_read_only, bool p_include_current_values) {
	...
	content += "Use `property_path` exactly with scene.set_property. For subproperty edits, use one of the listed subproperty_paths when present.\n";
	content += "Resource properties accept null, {\"resource_path\":\"res://...\"}, or {\"resource_type\":\"Type\",\"properties\":{...}}. Nested Resource properties can be edited with paths like shape:size after a resource exists.\n";
...
AISceneEditingResult AISceneEditingService::_save_current_scene_request_main_thread() {
	AISceneEditingResult result;
	String error;
	Node *scene = _get_edited_scene(error);
	if (!scene) {
	...
	result.success = true;
	result.message = vformat("Saved current scene `%s`.", saved_path);
	result.metadata["scene_path"] = saved_path;
	result.metadata["saved"] = true;
	return result;
}

AISceneEditingResult AISceneEditingService::_open_scene_main_thread(const String &p_path) {
	AISceneEditingResult result;
	String error;
	String scene_path;
	if (!_normalize_scene_save_path(p_path, scene_path, error)) {
	...
	EditorNode *editor = EditorNode::get_singleton();
	if (!editor) {
		result.error = "EditorNode is not available.";
		return result;
	}

	Error err = editor->open_scene(scene_path);
	if (err != OK) {
		result.error = vformat("Failed to open scene `%s` (error %d).", scene_path, err);
		return result;
	}

	result.success = true;
	result.message = vformat("Opened scene `%s`.", scene_path);
	result.metadata["scene_path"] = scene_path;
	return result;
}
...
AISceneEditingResult AISceneEditingService::rename_node(const String &p_node_path, const String &p_new_name) {
	MainThreadRequest request;
	request.operation = MainThreadRequest::OP_RENAME_NODE;
	request.node_path = p_node_path;
	request.new_name = p_new_name;
	return _dispatch_to_main_thread(request);
}

AISceneEditingResult AISceneEditingService::move_node(const String &p_node_path, const String &p_new_parent_path, int p_position) {
	MainThreadRequest request;
	request.operation = MainThreadRequest::OP_MOVE_NODE;
	request.node_path = p_node_path;
	request.new_parent_path = p_new_parent_path;
	request.position = p_position;
	return _dispatch_to_main_thread(request);
}

AISceneEditingResult AISceneEditingService::set_property(const String &p_node_path, const String &p_property_path, const Variant &p_value) {
	MainThreadRequest request;
	request.operation = MainThreadRequest::OP_SET_PROPERTY;
	request.node_path = p_node_path;
	request.property_path = p_property_path;
	request.value = p_value;
	return _dispatch_to_main_thread(request);
}
...
AISceneEditingResult AISceneEditingService::save_current_scene() {
	MainThreadRequest request;
	request.operation = MainThreadRequest::OP_SAVE_CURRENT_SCENE;
	return _dispatch_to_main_thread(request);
}

AISceneEditingResult AISceneEditingService::open_scene(const String &p_path) {
	MainThreadRequest request;
	request.operation = MainThreadRequest::OP_OPEN_SCENE;
	request.scene_path = p_path;
	return _dispatch_to_main_thread(request);
}
