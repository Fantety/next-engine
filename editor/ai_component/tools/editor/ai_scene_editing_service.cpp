...
AISceneEditingResult AISceneEditingService::_set_property_main_thread(const String &p_node_path, const String &p_property_path, const Variant &p_value) {
	...
	Variant converted_value;
	String property_type;
	if (!_prepare_property_assignment(node, p_node_path, p_property_path, p_value, property_path, current_value, converted_value, property_type, error)) {
		result.error = error;
		return result;
	}
	...
	result.metadata["node_path"] = p_node_path;
	result.metadata["property_path"] = p_property_path;
	result.metadata["property_type"] = property_type;
	...
	for (int i = 0; i < ops.size(); i++) {
		...
		if (op == "add_node" || op == "instantiate_scene") {
			...
			if (create_mode) {
				...
				return fail(vformat("ops[%d].op: delete_node is only supported for existing current scenes.", i));
			}
			const String node_locator = String(op_dict.get("node", "")).strip_edges();
			if (node_locator.is_empty()) {
				return fail(vformat("ops[%d].node is required.", i));
			}
			if (patch_nodes.has(node_locator)) {
				return fail(vformat("ops[%d].node: delete_node cannot target nodes created earlier in the same patch.", i));
			}
			Node *node = _resolve_node_path(scene, node_locator, false, error);
			if (!node) {
				return fail(vformat("ops[%d].node: %s", i, error));
			...
			if (node->is_internal()) {
				return fail(vformat("ops[%d].node: Internal nodes cannot be deleted by the AI scene patch tool.", i));
			}
			Node *parent = node->get_parent();
			if (!parent) {
				return fail(vformat("ops[%d].node has no parent.", i));
			...
			undo_redo->add_undo_reference(node);
			op_result["node"] = node_locator;
			last_selected_node = parent;
		} else {
			if (create_mode && scene && !scene_added_to_editor) {
				memdelete(scene);
				scene = nullptr;
			}
			return fail(vformat("ops[%d].op `%s` is not supported. Supported ops: add_node, instantiate_scene, set_property, set_properties, rename_node, move_node, delete_node.", i, op));
		}
