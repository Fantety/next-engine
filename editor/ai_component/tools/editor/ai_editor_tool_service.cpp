...
Node *AIEditorToolService::_resolve_node_path(Node *p_scene_root, const String &p_path, bool p_allow_root, String &r_error, bool p_disallow_property_subpaths, const String &p_not_found_error) const {
...
bool AIEditorToolService::_ensure_project_parent_directory(const String &p_path, const String &p_resource_label, String &r_error) const {
...
bool AIEditorToolService::_ensure_editor_filesystem_parent_directory(const String &p_path, const String &p_resource_label, String &r_error) const {
	const String base_dir = p_path.get_base_dir();
	if (base_dir.is_empty() || base_dir == "res://") {
		return true;
	}
...
bool AIEditorToolService::_save_current_scene_with_packed_scene_main_thread(Node *p_scene, const String &p_unsaved_error, String &r_saved_path, String &r_error) const {
...
bool AIEditorToolService::_save_current_scene_with_editor_main_thread(Node *p_scene, const String &p_unsaved_error, String &r_saved_path, String &r_error) const {
	ERR_FAIL_NULL_V(p_scene, false);
...
void AIEditorToolService::_refresh_file_system(const String &p_path) const {
	if (!Thread::is_main_thread()) {
		if (MessageQueue::get_main_singleton()) {
			MessageQueue::get_main_singleton()->push_callable(callable_mp(const_cast<AIEditorToolService *>(this), &AIEditorToolService::_refresh_file_system), p_path);
		}
		return;
	}
...
void AIEditorToolService::_update_scene_tree() const {
...
Node *AIEditorToolService::resolve_scene_node_path_for_test(Node *p_scene_root, const String &p_path, bool p_allow_root, String &r_error) {
	Ref<AIEditorToolService> service;
	service.instantiate();
