...
def md_to_cpp_string(md_content):
    try:
        # 转义特殊字符（注意处理顺序）
        escaped = md_content.replace('\\', '\\\\')  # 必须先处理反斜杠
...
def main():
    if len(sys.argv) != 3:
        print("Usage: python generate_md_header.py <input.md> <output.h>")
        sys.exit(1)
    ...
    try:
        ...
        # 转换为C++字符串
        cpp_string = md_to_cpp_string(md_content)
        
        # 变量命名
        namespace = "AITools"
        var_name = "SYSTEM_PROMPT_STR"
        guard_macro = "SYSTEM_PROMPT_STR_H"
