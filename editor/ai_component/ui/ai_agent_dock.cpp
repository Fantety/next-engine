...
AIProviderConfig AIAgentDock::_get_provider_config(const String &p_model) const {
	...
	config.api_key = EditorSettings::get_singleton()->has_setting("deepseek/api_key") ? String(EditorSettings::get_singleton()->get("deepseek/api_key")) : String();
	config.base_url = EditorSettings::get_singleton()->has_setting("deepseek/url") ? String(EditorSettings::get_singleton()->get("deepseek/url")) : String("https://api.deepseek.com/v1");
	return config;
}
