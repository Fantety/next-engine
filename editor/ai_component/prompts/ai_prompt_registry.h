...
static constexpr const char *SYSTEM_PROMPT =
	...
	"Tools use progressive disclosure. Always-available tools and the active tool category are listed in Tool disclosure context. If a needed domain tool is not exposed, call agent.activate_tool_category with one available category name; the selected category replaces the previous active category while always-available tools remain available.\n"
